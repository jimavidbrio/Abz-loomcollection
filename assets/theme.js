(function ($) {
  ...
  */
    ...
    $formContainer.find('form.product-purchase-form').each(function () {
      ...
      $form.on('submit', function (evt) {
        ...
        if (theme.settings.after_add_to_cart !== 'no-js' && $formContainer.is('[data-ajax-add-to-cart="true"]')) {
          ...
          var hasBulb1 = $('.Bulbselected1').attr('data-item-id') !== undefined;
          var hasBulb2 = $('.Bulbselected2').attr('data-item-id') !== undefined;

          if (hasBulb1 || hasBulb2) {
            var items = [];
            const genRanHex = size => [...Array(size)].map(() => Math.floor(Math.random() * 16).toString(16)).join('')
            const bundleCode = genRanHex(6);
            var mainid, qty;
            ...
            items.push({
              ...
              'properties': {
                '_randomNumber': bundleCode,
                '_main': true
              }
            });
            if (hasBulb1) {
              var subid1 = $('.Bulbselected1').attr('data-item-id');
              var subqty1 = $('.Bulbselected1').attr('data-bulb-qty');
              subqty1 = parseInt(subqty1);
              var mainqty1 = parseInt(qty);
              var result1 = subqty1 * mainqty1;
              items.push({
                'id': parseInt(subid1),
                'quantity': parseInt(result1),
                'properties': {
                  '_randomNumber': bundleCode,
                  '_sub': 'upsell1'
                }
            ...
            if (hasBulb2) {
              var subid2 = $('.Bulbselected2').attr('data-item-id');
              var subqty2 = $('.Bulbselected2').attr('data-bulb-qty');
              subqty2 = parseInt(subqty2);
              var mainqty2 = parseInt(qty);
              var result2 = subqty2 * mainqty2;
              console.log('subqty2======>',subqty2);
              console.log('mainqty2======>',mainqty2);
              console.log('result2======>',result2);
              items.push({
                'id': parseInt(subid2),
                'quantity': parseInt(result2),
                'properties': {
                  '_randomNumber': bundleCode,
                  '_sub': 'upsell2'
                }
            ...
            fetch(window.Shopify.routes.root + 'cart/add.js', {
              ...
              body: JSON.stringify(formDataBundle)
            })
            .then(response => {
              return response.json();
            })
            .then(response => {
              theme.addedToCartHandler($form, 'bundle');
            })
            .catch((error) => {
              //Enable add button   
  ...
  theme.addedToCartHandler = function (response, bundle) {
    ...
    if(bundle == 'bundle') {
      console.log('bundle======');
      $(response).find('button[type="submit"]').removeAttr('disabled').each(function () {
        $btn = $(this);
  ...
  var CartForm = class extends HTMLElement {
    connectedCallback() {
      ...
      if (this.enableAjaxUpdate) {
        ...
        this.querySelectorAll('[data-remove-bundle]').forEach(button => {
          button.addEventListener('click', function (e) {
            e.preventDefault();
            let updates = {};
            var key = e.currentTarget.dataset.key;
            updates[key] = 0;
            var currentTargetElemet = e.currentTarget.closest('.cart-item');
            var lineElement = currentTargetElemet.querySelectorAll('.upsell-item');
            lineElement.forEach(line => {
                var key = line.dataset.key;
                updates[key] = 0;
            });                        
            cartFormCustom.adjustItemQuantity(e.target.closest('.cart-item'), { to: 0 }, updates);              
          })
        })
        ...
        theme.addDelegateEventListener(this, 'click', '.cart-item__remove', (evt) => {
          ...
          updates[key] = 0;
          this.adjustItemQuantity(evt.target.closest('.cart-item'), { to: 0 }, updates);
        });

        theme.addDelegateEventListener(this, 'click', '.quantity-down', (evt) => {
          evt.preventDefault();
          let updates = {};
          var item = evt.target.closest('.cart-item');
          var quantityInput = item.querySelector('.cart-item__quantity-input');
          
          var key = evt.target.dataset.key;
          if (typeof evt.target.dataset.key) { // added by Wholesale Helper
            key = item.querySelector('.remove-link.cart-item__remove').dataset.key;
          }
          var newQuantity = parseInt(quantityInput.value);
          
          updates[key] = newQuantity - 1;
          this.adjustItemQuantity(evt.target.closest('.cart-item'), { decrease: true }, updates);
        });
        ...
        theme.addDelegateEventListener(this, 'click', '.quantity-up', (evt) => {
          ...
          updates[key] = newQuantity + 1;
          this.adjustItemQuantity(evt.target.closest('.cart-item'), { increase: true }, updates);
        });
    ...
    adjustItemQuantity(item, change, updates) {
      var quantityInput = item.querySelector('.cart-item__quantity-input');
  ...
  theme.LoadFilterer = function (section) {
    ...
    if (document.querySelector('.utility-bar')) {
      ...
      $('.filter-container', this.$container).addClass('filter-container--show-filters-desktop');
      $('.toggle-btn[data-toggle-filters]', this.$container).addClass('toggle-btn--revealed-desktop');
      
      // Event handler to toggle filters visibility
      this.$container.on('click' + this.namespace, '[data-toggle-filters]', function (evt) {
        var isNowVisible = $('.filter-container', this.$container).toggleClass('filter-container--show-filters-desktop filter-container--show-filters-mobile').hasClass('filter-container--show-filters-desktop');
        $('.toggle-btn[data-toggle-filters]', this.$container).toggleClass('toggle-btn--revealed-desktop', isNowVisible);
        // handle resized collection grid
  ...
  theme.OptionManager = new function () {
    ...
    _.getProductData = function ($form) {
      ...
      if (!theme.productData[productId]) {
        ...
        console.log('productData=====>', theme.productData[productId]);
      }
      data = theme.productData[productId];
      console.log('dataproduct======>', data);

      var selectbulb1 = $('.Bulbselected1').data('item-id');
      var selectbulb2 = $('.Bulbselected2').data('item-id');
      console.log('selectbulb1=======>', selectbulb1);
      console.log('selectbulb2=======>', selectbulb2);

      // if(!theme.productbulbData[selectbulb]){
      // theme.productbulbData[selectbulb] = JSON.parse(document.getElementById('ProductJson-' + selectbulb).innerHTML);
      
      // }

      // newbulb = theme.productbulbData[selectbulb];
      // console.log('nathiaavtu=====>',  newbulb );
    ...
    _.updateVariantDeterminedVisibilityAreas = function (variant, $container) {
      $container.find('.variant-visibility-area').each(function () {
        ...
        try{
          $('.current-price').removeClass("wpd-processed");
          $('.current-price').attr("data-wpd-variant-compare-at-price",variant.compare_at_price == null ? "":variant.compare_at_price);
          $('.current-price').attr("data-wpd-variant-price",variant.price);
          $('.current-price').attr("data-wpd-variant-id",variant.id);
          $('.current-price').attr("data-wpd-product-handle",window.wpdExtensionIsProductPage.handle);
        }catch(e){}
  ...
  $(function ($) {
    ...
    $('.customSwatches a').hover(
      function() {
        var colorName = $(this).attr('data-color-name');
        $(this).closest('.option-selector-fieldset').find('.label__value').html(colorName)
      }, function() {
        var colorName  = $('.customSwatches a:eq(0)').attr('data-color-name');
        $(this).closest('.option-selector-fieldset').find('.label__value').html(colorName)
      }
    );
  ...
  // Upsell 1 logic
  var defaultOption1 = $('.option_value.upsell1:first');
  defaultOption1.addClass('Bulbselected1');
  $('.current-option-img.upsell1').attr('src', defaultOption1.find('.upsell-img img').attr('src'));
  $('#upsell-detail-1').text(defaultOption1.text());
  var selectedItemId1 = defaultOption1.data('item-id');
  var selectedno1 = defaultOption1.attr('data-value');
  if(selectedno1 == 'no'){
    $('.current-option-img.upsell1').addClass('no-small-img');
    $('.current-option-img-wrapper.upsell1').addClass('current-img-wrapper');
  } else {
    $('.current-option-img.upsell1').removeClass('no-small-img');
    $('.current-option-img-wrapper.upsell1').removeClass('current-img-wrapper');
  }
  $('.current-option.upsell1').click(function(){
    $('.upsell1-select').toggleClass('open');
  });
  $('.option_value.upsell1').click(function(){
    $('.option_value.upsell1').removeClass('Bulbselected1');
    $(this).addClass('Bulbselected1');
    var optionImgSrc = $(this).find('.upsell-img img').attr('src');
    $('.current-option-img.upsell1').attr('src', optionImgSrc);
    $('#upsell-detail-1').text($(this).text());
    $('.upsell1-select').removeClass('open');
    selectedItemId1 = $(this).data('item-id');
    var nonevalue1 = $('.Bulbselected1').data('value');
    if(nonevalue1 == 'no'){
      $('.current-option-img.upsell1').addClass('small-img');
      $('.current-option-img-wrapper.upsell1').addClass('small-img-wrapper');
    } else {
      $('.current-option-img.upsell1').removeClass('small-img');
      $('.current-option-img-wrapper.upsell1').removeClass('small-img-wrapper');
    }
  });

  // Upsell 2 logic
  var defaultOption2 = $('.option_value.upsell2:first');
  defaultOption2.addClass('Bulbselected2');
  $('.current-option-img.upsell2').attr('src', defaultOption2.find('.upsell-img img').attr('src'));
  $('#upsell-detail-2').text(defaultOption2.text());
  var selectedItemId2 = defaultOption2.data('item-id');
  var selectedno2 = defaultOption2.attr('data-value');
  if(selectedno2 == 'no'){
    $('.current-option-img.upsell2').addClass('no-small-img');
    $('.current-option-img-wrapper.upsell2').addClass('current-img-wrapper');
  } else {
    $('.current-option-img.upsell2').removeClass('no-small-img');
    $('.current-option-img-wrapper.upsell2').removeClass('current-img-wrapper');
  }
  $('.current-option.upsell2').click(function(){
    $('.upsell2-select').toggleClass('open');
  });
  $('.option_value.upsell2').click(function(){
    $('.option_value.upsell2').removeClass('Bulbselected2');
    $(this).addClass('Bulbselected2');
    var optionImgSrc = $(this).find('.upsell-img img').attr('src');
    $('.current-option-img.upsell2').attr('src', optionImgSrc);
    $('#upsell-detail-2').text($(this).text());
    $('.upsell2-select').removeClass('open');
    selectedItemId2 = $(this).data('item-id');
    var nonevalue2 = $('.Bulbselected2').data('value');
    if(nonevalue2 == 'no'){
      $('.current-option-img.upsell2').addClass('small-img');
      $('.current-option-img-wrapper.upsell2').addClass('small-img-wrapper');
    } else {
      $('.current-option-img.upsell2').removeClass('small-img');
      $('.current-option-img-wrapper.upsell2').removeClass('small-img-wrapper');
    }
  });

  // Remove old global .option_value/.Bulbselected logic
...
window.onload = function() {
   ...
   var shippingElement = document.querySelector('.shipping-delivery');

    if (shippingElement !== null) {
        var encodedData = shippingElement.innerHTML;
        // console.log('encodedData=======>', encodedData);
        var formattedData = removeHTMLTags(encodedData);
        
        // console.log('formattedData=======>', formattedData);
        shippingElement.innerHTML = formattedData;
    }

  
    //click here
  var careElement = document.querySelector('.care-maintenance');

    if (careElement !== null) {
        var careData = careElement.innerHTML;
        // console.log('careData=======>', careData); 
        var careformatData = removeHTMLTags(careData);
        //console.log('careformatData=======>', careformatData); 
        var anchorTag = document.createElement('a');
        
        // Set the inner text of the anchor tag
        anchorTag.innerText = 'click here.';
        anchorTag.href = 'https://cdn.shopify.com/s/files/1/0161/5003/6580/files/004_TLC_Care_Instructions.pdf?v=1661093572';
        anchorTag.title = 'The loom Collection - Care and maintenance Instructions';
        anchorTag.target = '_blank';
        careData += ' ' + anchorTag.outerHTML;
        careElement.innerHTML = careData;
    }
    ...
    if(screen.width <= 767) {
        var originalPositionBottom = document.getElementById('add-to-cart-button').getBoundingClientRect().bottom;
        console.log('originalPositionBottom========>',originalPositionBottom);
        if(originalPositionBottom >= window.innerHeight) {
          document.querySelector('.add-to-cart')?.classList.add('sticky--add-to-cart')
        }
      ...
      document.addEventListener('scroll', function() {
          ...
          // STICKY ADD TO CART
          var originalPositionTop = document.getElementById('add-to-cart-button').getBoundingClientRect().top;
          // console.log('originalPositionTop========>',originalPositionTop);
          var originalPositionBottom = document.getElementById('add-to-cart-button').getBoundingClientRect().bottom;
          // console.log('originalPositionBottom========>',originalPositionBottom);
          var stickyPosition = document.querySelector('.add-to-cart').getBoundingClientRect().top;
          // console.log('stickyPosition========>',stickyPosition);
      
          if(stickyPosition >= (originalPositionTop - 16)) {
            document.querySelector('.add-to-cart')?.classList.remove('sticky--add-to-cart')
          }
          if(originalPositionBottom >= window.innerHeight) {
            document.querySelector('.add-to-cart')?.classList.add('sticky--add-to-cart')
          }
